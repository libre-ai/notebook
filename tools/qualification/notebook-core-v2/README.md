...
The resource manifest requires only the physically qualified 32+ GiB arm64 class for the current Gate B scope. The 8 GiB and 16–24 GiB classes remain optional community observations and are not claimed as supported without their own reviewed evidence. Gate B remains rejected until attributable real browser-process OOM and separate exact-product-host architecture, security, cryptography, privacy and performance reviews pass on an immutable candidate.
