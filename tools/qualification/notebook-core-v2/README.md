...
The resource manifest targets constrained and mainstream arm64 desktops, but only the high-memory reference has archived evidence; the 8 GiB and 16–24 GiB classes remain pending real hardware. Gate B remains rejected until attributable real browser-process OOM, the remaining device classes, and separate exact-product-host architecture, security, cryptography, privacy and performance reviews pass on an immutable candidate.
