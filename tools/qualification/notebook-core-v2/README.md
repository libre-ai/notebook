...
- one import-free compiled `WebAssembly.Module` per page, structured-cloned into a fresh worker and instantiated anew for every operation; compilation occurs after the RSS baseline and its measured duration is conservatively added to every end-to-end sample;
- byte-for-byte verification of the public 16 MiB `0x5a` open result against its precomputed SHA-256, avoiding a qualification-only 16 MiB digest copy; self-close plus host termination on every worker outcome, with one event-loop teardown yield included in end-to-end timing;
...
The compiled module is immutable and capability-free; reusing it does not reuse linear memory or an instance. Trap and internal-fault campaigns continue to use their dedicated disposable workers and artifacts rather than this performance cache.
